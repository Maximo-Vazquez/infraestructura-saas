...
- Certificado TLS wildcard en un Secret llamado `wildcard-mydominio-tls` con SAN para `mi-nas-vaz.myqnapcloud.com` y `*.mi-nas-vaz.myqnapcloud.com` (o emítelo con cert-manager).
...
- Si ves certificado no confiable: crea/corrige el Secret `wildcard-mydominio-tls` en `default`.
